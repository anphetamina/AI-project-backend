...
public interface CourseRepository extends JpaRepository<Course, String> {
    @Query("SELECT s FROM Student s INNER JOIN s.teams t INNER JOIN t.course c WHERE c.id=:courseId AND t.status=it.polito.ai.backend.entities.TeamStatus.ACTIVE")
    List<Student> getStudentsInTeams(String courseId);
    @Query("SELECT s FROM Student s INNER JOIN s.courses c WHERE c.id=:courseId AND s NOT IN (SELECT s FROM Student s INNER JOIN s.teams t INNER JOIN t.course c WHERE c.id=:courseId AND t.status=it.polito.ai.backend.entities.TeamStatus.ACTIVE)")
